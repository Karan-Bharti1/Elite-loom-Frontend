...
const Cart = () => {
    ...
    const displayProducts = randomProducts
        ?.map(item => (
                    ...
                    <div className="card-body">
                        ...
                        <p className="card-text">
                            ...
                            <span>
                                ...
                                <span className="text-danger fw-bold">{item.discountPercentage}% off</span>
                            </span>
                        </p>
                    </div>
                </div>
        ...
        ?.map((item) => (
            <div key={item._id} className="row my-3 border-0">
                ...
                <div className="col-md-8 text-center py-1">
                    ...
                    <button onClick={() => handleClickIncrease(item._id)}>+</button >
                    <span className="px-2">{quantity[item._id]}</span>
                    <button onClick={() => handleClickDecrease(item._id)}>-</button>
                    <span className="mx-3">Selected Size :
                        <span className="text-danger">{item.selectedSize}</span>
                    </span>
                    <br/><br/>
                    <button
                        className=" btn btn-danger p-1 m-1"
                        onClick={() => handleDelete(item._id)}>Delete Item</button>
                    <button
                        className=" btn btn-danger p-1 m-1"
                        onClick={() => handleWishlist(item._id, item.productDetails._id)}>Move to wishlist</button>
                </div>
            </div>
        ))
    const displayProductDetails = cartData
        ?.map(item => (
    ...
    const subtotal = cartData
        ?.reduce((acc, curr) => acc + (curr.productDetails.price - (curr.productDetails.price * curr.productDetails.discountPercentage) / 100) * quantity[curr._id], 0)

    return (
    <> <Header/>
        {
        deleteItemAlert.visible && <span
                id="alert"
                className="top-10 end-0 text-white bg-danger position-fixed p-3 m-3">{deleteItemAlert.message}</span>
    }
    {
        quantityAlert.visible && <span
                id="alert"
                className="top-10 end-0 text-white bg-danger position-fixed p-3 m-3">{quantityAlert.message}</span>
    }
    {
        moveToWishlistAlert.visible && <span
                id="alert"
                className="top-10 end-0 text-white bg-danger position-fixed p-3 m-3">{moveToWishlistAlert.message}</span>
...
{!loading && !error && cartData.length > 0 &&< div className = "row" > <div className="col-md-6">
    ...
    {displayCartItems}
</div> < div className = "col-md-6 " > <div className="border-0 mt-5">
...
</div> < div className = "text-center py-4" > <Link className="btn btn-danger" to="/chooseaddress">Select Address</Link> < /div>

</div > </div>
}
{!loading && cartData.length === 0 && ( <> <div>
    <h1 className="text-center">
        No Items present in your Cart .</h1>
    <div className="text-center">
        <Link className="btn btn-danger fw-bold my-3" to="/products">Explore us</Link>
    </div>
    <div className="row">
        {displayProducts}
    </div>
